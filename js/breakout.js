/**
 * /js/breakout.js — hands an in-app webview visitor to their real browser before the
 * lander, because that webview fails on real devices.
 *
 * THE RULES THIS FILE MUST HOLD TO, and they are not negotiable:
 *   1. ONE destination for everyone. Nothing about the visitor ever changes WHERE they
 *      go — only which MECHANISM opens it.
 *   2. ONE page for everyone. The markup renders fully before this runs and is never
 *      hidden, blanked or swapped. No display:none gate, no document.write.
 *   3. NO visitor scoring of any kind. No review detection, no webdriver test, no
 *      fingerprint. A crawler gets the same page and the same destination as a buyer.
 *   4. The user agent is read for exactly ONE reason: an OS URL scheme only exists on
 *      one OS, and firing it in a real browser shows an error page. Every real browser
 *      is sent straight through.
 *
 * The full rationale — including what was removed and why — lives in the lander
 * playbook, which is not served. This file is public; that history does not belong in it.
 */
(function () {
  'use strict';
